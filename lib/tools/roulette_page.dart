...
class _RoulettePage extends State<RoulettePage> with TickerProviderStateMixin {
  ...
  // Replaced Yaru colors with Material-3 vibrant colors
  ...
  Future<void> saveRouletteUnitsToSharedPreferences() async {
    ...
    prefs.setStringList(
      SHARED_PREFERENCES_TOOL_ROULETTE_ITEMS,
      rouletteItemsToSave,
    );
  }
  ...
  Future<void> addRouletteUnit(String text) async {
    if (rouletteItems.length >= 8) {
      showOkTextDialog(
        context,
        t.generic.warning,
        t.tools.roulette.warning
            .you_cant_add_more_than_x_items(numberOfItems: 8),
      );
      return;
    }
  ...
  Future<void> removeRouletteUnit(int index) async {
    if (rouletteItems.length <= 2) {
      showOkTextDialog(
        ...
        t.generic.warning,
        t.tools.roulette.warning
            .you_must_have_at_least_x_items(numberOfItems: 2),
      );
      return;
    }
          ...
          content: SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return ListTile(
                  title: Text(
                    "${index + 1}) ${(rouletteItems[index].child as Text).data.toString()}",
                  ),
                  onTap: () async {
                    Navigator.pop(context);
                  ...
                  children: [
                    ...
                    ConstrainedBox(
                      ...
                      child: GestureDetector(
                        ...
                        child: FortuneWheel(
                          ...
                          onFling: rollRoulette,
                          onAnimationStart: () => isSpinning = true,
                          onAnimationEnd: () => isSpinning = false,
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: FilledButton(
                            onPressed: showAddRouletteUnitDialog,
                            child: Text(t.tools.roulette.add),
                          ),
                        ),
                        ...
                        Expanded(
                          child: FilledButton(
                            onPressed: showRemoveRouletteChoiceListDialog,
